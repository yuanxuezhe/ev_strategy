# 旧策略 DSL 与三端转译(已下线, 2026-09 归档)

> **本文档已归档, 不再更新。** 仅为历史提交 / commit 5585371 之前的代码形态留档。
> 当前策略契约详见 [04-策略](../04-策略.md) §1 与 `evtrade/strategies/vectorized_base.py`。

旧策略 DSL 与三端转译(docstring DSL body + numba `@njit` 流式内核 +
NVRTC CUDA `__device__` 渲染器 + `state_spec` AST 白名单三端投影)已于 2026-09 整体下线,
对应代码(`evtrade/strategies/dsl.py`、`core/kernel.py`、
...
写新策略请读 [04-策略 §1](../04-策略.md) + [06-扩展指南 §2](../06-扩展指南.md);
契约细节见 `evtrade/strategies/vectorized_base.py`。
...
- 策略主逻辑写在 `compute_signal` 的 **docstring**(DSL body),
  由 AST 白名单(`_CALL_WHITELIST`) + 三端渲染器(Python exec / numba `@njit` / CUDA C99)
  自动转译。
...
| 2026-09 (~10) | `consolidate-simplify-core` | 过渡期入口 `compute_signals` 也删除; 三端投影 / `state_spec` 无残留 |
...
| 一致性保证 | 三端 bitwise(`--fmad=false`) | 两路径同 `step`, 浮点路径一致(reconcile 锁定, 现已下线) |

## 4. 迁移指引(旧 DSL 策略 → 现行契约)

1. 把 DSL body 里的指标递推(如 EMA)改写为 `step` 内调用
   `ema_step(state, value, p)` / `ema_channel_step(state, h, l, p)`, state 收敛进一个 `@dataclass`。
2. 把 FSM / 锁存逻辑写成 `step` 内的纯函数 + state 字段; **不要**用 `self._xxx` instance attr。
...
5. 验证: `tests/test_strategy_unified.py`(step 状态跨调用持续 + CPU/GPU 对账)+
   `python -m evtrade backtest --strategy <key> --device cpu`(vectorized 路径端到端)。

## 5. 跨文档索引

- [04-策略 §1](../04-策略.md) — 策略 `step` 骨架 + 统一契约(channel_deviation / ma_crossover 范式)
- [06-扩展指南 §2](../06-扩展指南.md) — 新策略开发模板
- [07-PyTorch与性能](../07-PyTorch与性能.md) — PyTorch 统一后端(torch CPU/CUDA 单后端)
- `openspec/specs/evtrade-architecture/spec.md` — `R: Single contract = VectorizedStrategy.step`
- `openspec/changes/archive/2026-09-09-2026-09-09-decouple-indicators-from-framework/`
- `openspec/changes/archive/2026-09-09-2026-09-09-unify-strategy-contract/`
- `openspec/changes/archive/2026-09-10-2026-09-10-strategy-step-only/`
- `openspec/changes/archive/2026-09-10-2026-09-10-pytorch-unified-strategy/`
