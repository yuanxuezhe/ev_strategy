...
- [ ] 1.1 `evtrade/indicators/ema.py`: 加 `EMAState` / `EMAChannelState` dataclass; 新增 `ema_step(state, value, p) -> (state, ema)` 与 `ema_channel_step(state, h, l, p) -> (state, up, dw)`; **保留** `xp_ema` / `xp_ema_channel` (engine fast-path 用); 保留旧 `ema_push / ema_current / ema_channel_push / ema_channel_current` 作 deprecated shim (一行注释 + alias)
- [ ] 1.2 `evtrade/indicators/atr.py`: 加 `ATRState` dataclass; 新增 `atr_step(state, h, l, c, p) -> (state, atr)`; 保留 `xp_atr` / `true_range`; 旧 `atr_push / atr_current` 作 shim
- [ ] 1.3 `evtrade/indicators/rsi.py`: 加 `RSIState` dataclass; 新增 `rsi_step(state, close, p) -> (state, rsi)`; 保留 `xp_rsi`; 旧 `rsi_push / rsi_current` 作 shim
- [ ] 1.4 `evtrade/indicators/boll.py`: 加 `BollState` dataclass; 新增 `sma_step(state, value, p) -> (state, sma)` 与 `boll_step(state, value, p, k) -> (state, mid, upper, lower)`; 保留 `xp_sma` / `xp_bollinger`; 旧 `sma_push / sma_current / boll_push / boll_current` 作 shim
- [ ] 1.5 `evtrade/indicators/__init__.py`: re-export 新 `*_State` / `*_step`; 旧 `*_push / *_current` 标 `DeprecationWarning` (说明: "已合并到 *_step, 2026-09-10 后删除")

## 2. 基类 — step 抽象 + init_state

- [ ] 2.1 `evtrade/strategies/vectorized_base.py`:
...
- [ ] 3.1 `evtrade/strategies/channel_deviation.py`:
...
- [ ] 3.2 `evtrade/strategies/ma_crossover.py`:
...
- [ ] 5.1 `tests/test_strategy_unified.py`:
...
- [ ] 5.2 新增 `test_init_state_returns_dataclass`:
  - `ChannelDeviationStrategy().init_state({})` 是 `ChannelDeviationState`
  - `MACrossoverStrategy().init_state({})` 是 `MACrossoverState` 或 None
- [ ] 5.3 新增 `test_no_instance_state_in_strategies`:
  - 静态扫描 `evtrade/strategies/*.py` (排除 `vectorized_base.py`)
  - MUST NOT 出现 `self._fsm` / `self._up_st` / `self._dw_st` / `self._has_prev`
- [ ] 5.4 新增 `test_step_state_persists_across_calls`:
  - 调 step(state, bar1) -> (state', sig1); 再调 step(state', bar2) -> (state'', sig2)
  - 验证 EMA 单调正确 (state.ema.count > 0)
- [ ] 5.5 跑 `uv run pytest -q` 应 122+ 全 PASS

## 6. 删除旧 API（彻底清理）

- [ ] 6.1 `evtrade/strategies/vectorized_base.py`: 确认 `compute_signals` / `_for_one_bar` 不再有任何引用
- [ ] 6.2 `evtrade/strategies/channel_deviation.py`: 确认 `_parse_thresholds` / `_compute_devs_*` / `_run_fsm` 不再有任何引用
- [ ] 6.3 `evtrade/indicators/__init__.py`: 删除旧 `*_push / *_current` re-export (保留新 `*_step`)
- [ ] 6.4 跑 `grep -rn 'compute_signals_for_one_bar\|ema_push\|ema_current\|ema_channel_push\|ema_channel_current\|atr_push\|atr_current\|rsi_push\|rsi_current\|sma_push\|sma_current\|boll_push\|boll_current' evtrade/` 应只剩 deprecation shim 的 alias 行 (或在测试 fixtures 里)
- 6.5 跑 `uv run pytest -q` 仍 122+ 全 PASS

## 7. KB 同步

- [ ] 7.1 `kbs/README.md`: 术语表 (line 93-94) 改: `compute_signals(xp, bars, params)` -> `step(state, bar, params) -> (state, sig)`; 删除 `_for_one_bar` 行
- [ ] 7.2 `kbs/使用说明.md`: 模板代码 (line 149-151) 改 step
- [ ] 7.3 `kbs/14-策略DSL与三端转译.md`: 整体重写: 单方法 step + state 由 engine 持有 + 指标 step API
- [ ] 7.4 `kbs/12-重构与性能内核.md`: §2.3 (line 87-93) 状态存放表重写: vectorized 用 list of state; Engine 用 self._state
- [ ] 7.5 `kbs/09-引擎Engine与主流程.md`: §2 (line 19-20, 35, 49-53) 改 Engine.on_bars 调 step; §7 (line 150) 删 EMA 漂移说明 (state 一致后无漂移)
- [ ] 7.6 `kbs/06-交易策略详解.md`: §5 (line 80-175) 整体改: 单一 step 骨架 + ChannelDeviationState dataclass

## 8. spec delta 合并

- [ ] 8.1 把 `openspec/changes/2026-09-10-strategy-step-only/specs/evtrade-architecture/spec.md` 内容手动合并到 `openspec/specs/evtrade-architecture/spec.md`:
...
- [ ] 8.2 跑 `openspec validate --specs` 通过 (无 openspec CLI 时人工核对: 8 Requirements + ~25 Scenarios)

## 9. 端到端验证

- [ ] 9.1 `uv run pytest -q` 122+ 全 PASS
- [ ] 9.2 `uv run python -m evtrade backtest --strategy channel_deviation --device cpu --period 5m --start 20250101 --end 20260903 --no-sleep`:
  - cagr/calmar/mdd/n_trades 与重构前 (498fb6a commit 数据) bitwise 一致
- [ ] 9.3 同上 `--device gpu`: cagr/calmar/mdd/n_trades bitwise 一致
- [ ] 9.4 `uv run python -m evtrade backtest --strategy ma_crossover --device cpu --period 5m ...`: 同上一致
- [ ] 9.5 `uv run python -m evtrade sweep --strategy channel_deviation --device cpu --max-mdd 0.15 --grid low1=0.5,1.0 --splits 20250601`: filter_pass 拒 mdd > 0.15 (与重构前一致)
- [ ] 9.6 `grep -rn "compute_signals_for_one_bar\|compute_signals(self" evtrade/` 应只剩 deprecation shim (理论上 0 命中)

## 10. openspec archive

- [ ] 10.1 `mv openspec/changes/2026-09-10-strategy-step-only openspec/changes/archive/`
- [ ] 10.2 git commit:
  ```
...
- [ ] 10.3 git push origin cupy-unified